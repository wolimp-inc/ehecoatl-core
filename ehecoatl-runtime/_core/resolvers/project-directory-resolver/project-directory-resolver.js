// _core/resolvers/project-directory-resolver/project-directory-resolver.js
...
class ProjectDirectoryResolver extends AdaptableUseCase {
  storageService;
  sharedCacheService;
  rpcEndpoint;
  ...
  constructor(kernelContext) {
    super(kernelContext.config._adapters.projectDirectoryResolver);
    this.config = kernelContext.config.adapters.projectDirectoryResolver;
    ...
    this.projectRegistryResolver = kernelContext.useCases?.projectRegistryResolver ?? null;
    this.routeMatcherCompiler = kernelContext.useCases?.projectRouteMatcherCompiler ?? null;
    ...
    this.spawnProjectAppAfterScan = this.config.spawnProjectAppAfterScan === true
      || this.config.spawnTenantAppAfterScan === true;
  ...
  attachProjectRegistryResolver(projectRegistryResolver) {
    this.projectRegistryResolver = projectRegistryResolver;
    return projectRegistryResolver;
  }

  attachTenantRegistryResolver(projectRegistryResolver) {
    return this.attachProjectRegistryResolver(projectRegistryResolver);
  }
  ...
  async scanRegistry() {
    try {
      const scanSummary = await this.adapter.scanProjectsAdapter({
        config: {
          ...this.config,
  ...
  async runScanCycle() {
    ...
    try {
      ...
      await this.#logRescanTenantsLine(`Project scan adapter completed in ${Date.now() - scanStartedAt}ms`);
      const reconcileStartedAt = Date.now();
      const reconcileResult = await this.projectRegistryResolver?.reconcileRegistry?.(this.registry, scanSummary) ?? {
        registry: this.registry,
        scanSummary
      };
      this.registry = reconcileResult.registry ?? this.registry;
      scanSummary = reconcileResult.scanSummary ?? scanSummary;
      await this.#logRescanTenantsLine(`Project registry reconciliation completed in ${Date.now() - reconcileStartedAt}ms`);
      const registryPersistStartedAt = Date.now();
      await this.projectRegistryResolver?.persistRegistry?.(this.registry, scanSummary);
      await this.#logRescanTenantsLine(`Project registry persistence completed in ${Date.now() - registryPersistStartedAt}ms`);
      ...
      await this.#logRescanTenantsLine(`Project shared-cache invalidation completed in ${Date.now() - invalidationStartedAt}ms`);
      const syncStartedAt = Date.now();
      await this.#syncProjectProcesses(scanSummary);
      await this.#logRescanTenantsLine(`Project process reconciliation completed in ${Date.now() - syncStartedAt}ms`);
      await this.#logRescanTenantsLine(`Project scan cycle completed in ${Date.now() - cycleStartedAt}ms`);
      return scanSummary;
    } finally {
      const clearMarkerStartedAt = Date.now();
      await this.#clearScanMarker();
      await this.#logRescanTenantsLine(`Project scan marker clear completed in ${Date.now() - clearMarkerStartedAt}ms`);
    }
  }

  async #logRescanTenantsLine(message) {
    const directorHooks = this.plugin?.hooks?.DIRECTOR;
    if (directorHooks?.RESCAN_PROJECTS === undefined || directorHooks.RESCAN_PROJECTS === null) {
      console.log(message);
      return;
    }

    await this.plugin.run(directorHooks.RESCAN_PROJECTS, {
      message,
      source: `project-directory-resolver`,
      stage: `project-scan`
  ...
  async waitUntilReady() {
    if (!this.runtime.firstScanPromise) {
      throw new Error(`ProjectDirectoryResolver first scan has not been started`);
    }

    await this.runtime.firstScanPromise;
    if (!this.runtime.ready) {
      throw new Error(`ProjectDirectoryResolver is not ready for route resolution`);
    }
  }
  ...
  async #syncProjectProcesses(scanSummary = {}) {
    if (!this.rpcEndpoint) return;

    if (this.spawnProjectAppAfterScan) {
      await this.#reconcileProjectProcesses(scanSummary);
    }
  ...
  async #reconcileProjectProcesses(scanSummary = {}) {
    await this.#reconcileProjectTransportProcesses(scanSummary);
    await this.#reconcileIsolatedRuntimeProcesses(scanSummary);
  }

  async #reconcileProjectTransportProcesses(scanSummary = {}) {
    const activeProjects = Array.isArray(scanSummary.activeProjects) ? scanSummary.activeProjects : [];
        ...
        pendingOperations.push((async () => {
          await this.#runSupervisorOperation({
            ...
            payload: {
              ...
              data: {
                label,
                reason: `project_scan_port_drift`
              }
            }
          });
          ...
          return this.#runSupervisorOperation({
            ...
            payload: {
              ...
              data: {
                reason: `project_scan_ensure`,
                layerKey: `projectScope`,
                processKey: `transport`,
                context: {
                  tenantId,
                  tenantDomain: activeProject?.tenantDomain ?? null,
                  tenantRoot: activeProject?.tenantRoot ?? null,
                  httpPort: activeProject?.internalProxy?.httpPort ?? null,
                  wsPort: activeProject?.internalProxy?.wsPort ?? null,
                  reason: `project_scan_ensure`
                }
              }
            }
    ...
    for (const processInfo of runningProcesses) {
      ...
      pendingOperations.push(this.#runSupervisorOperation({
        ...
        payload: {
          ...
          data: {
            label,
            reason: `project_scan_inactive_project`
          }
        }
      }));
    }
      ...
      if (isolatedRuntimeProcessHasDrift(runningProcess, activeHost)) {
        pendingOperations.push(this.#runSupervisorOperation({
          ...
          payload: {
            ...
            data: {
              label,
              reason: `project_scan_identity_drift`
            }
          }
        }));
      }
    ...
    for (const processInfo of runningProcesses) {
      ...
      pendingOperations.push(this.#runSupervisorOperation({
        ...
        payload: {
          ...
          data: {
            label,
            reason: `project_scan_inactive_host`
          }
        }
      }));
    }
  ...
  async #syncWebServerSources(scanSummary = {}) {
    ...
    const previousSources = buildProjectSourceMap(scanSummary.previousRegistry);
    const nextSources = buildProjectSourceMap(this.registry);
    console.log(`[WEB SERVER SYNC] previous=${previousSources.size} next=${nextSources.size}`);
    const changedSourceKeys = collectChangedProjectSourceKeys({
      previousSources,
      nextSources,
  ...
  async #askMainProcessRpc(payload = {}, {
    ...
    if (!this.rpcEndpoint) {
      throw new Error(`ProjectDirectoryResolver RPC endpoint is not ready`);
    }
    ...
    try {
      return await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => {
            reject(new Error(
              `ProjectDirectoryResolver RPC timeout after ${normalizedTimeoutMs}ms for question "${payload?.question ?? `unknown`}" `
            ));
          }, normalizedTimeoutMs);
          timer.unref?.();
        })
  ...
  async #runSupervisorOperation({
    ...
    try {
      ...
      console.warn(
        `Project supervisor ${action} failed for ${label}: ${normalizedError.message}`
      );
  ...
  async #awaitSupervisorOperations(pendingOperations, phaseLabel) {
    ...
    const message = failures
      ...
      .join(`; `);
    throw new Error(`Project ${phaseLabel} failed: ${message}`);
  }
}

function buildProjectSourceMap(registry) {
  ...
  for (const tenantRecord of registry.domains.values()) {
    ...
    const tenantApps = collectProjectApps(registry, tenantId);
    ...
    if (appRoutingMode === `path`) {
      ...
      for (const domain of tenantDomains) {
        const tenantKind = domain === tenantDomain ? `project-primary` : `project-alias`;
...
function collectProjectApps(registry, tenantId) {
...
function collectChangedProjectSourceKeys({
  previousSources,
  nextSources,
  initialScan
}) {
  ...
  for (const [sourceKey, nextSource] of nextSources.entries()) {
    ...
    if (stableSerializeProjectSource(previousSource) !== stableSerializeProjectSource(nextSource)) {
      keys.push(sourceKey);
    }
  }
  return keys;
...
function stableSerializeProjectSource(source) {
...
function transportProcessHasDrift(processInfo, activeProject) {
  if (!processInfo) return false;
  const variables = Array.isArray(processInfo.variables) ? processInfo.variables : [];
  const expectedHttpPort = String(activeProject?.internalProxy?.httpPort ?? ``);
  const expectedWsPort = String(activeProject?.internalProxy?.wsPort ?? ``);
  const expectedUser = String(activeProject?.transportProcessUser ?? ``);
  const expectedGroup = String(activeProject?.transportProcessGroup ?? ``);
  const expectedSecondGroup = String(activeProject?.transportProcessSecondGroup ?? ``);
  const expectedThirdGroup = String(activeProject?.transportProcessThirdGroup ?? ``);
...
module.exports = ProjectDirectoryResolver;
module.exports.buildProjectSourceMapForTests = buildProjectSourceMap;
